...
5. Choose the smallest change the evidence supports. Use `regression-test` when installed and a cheap local failing test can express the defect. Otherwise write the failing test first, run it to confirm the intended failure, make the minimum correction, and rerun it to green before cleanup.
