...
**Why:** Code is read far more than it is written. LOC, cyclomatic complexity, and "clean architecture" are proxies. Reader load is the thing that matters. The two axes are independent. A flat file with 50 globals can be as hard to reason about as a 6-layer adapter stack. Guard both. If `$principle-guard-the-context-window` is available, apply its equivalent guidance to agent context; otherwise keep large payloads out of the main context and retain only useful summaries. Working memory is finite for readers and agents.
