...
**Execution.** In a sweep, migration, or any run of similar edits, verify each change before starting the next. Each unit is a before/after bracket: known-good state, one change, run the check, then proceed. Start from a known clean baseline so every check measures the intended change. When a lever does the edits, the per-unit check is nearly free. Run it anyway.

**Delivery.** Order checkpoints so they prove the work. The canonical shape is a failing test first, then the fix. Other useful orders are a subtraction before the reshape, a baseline capture before the treatment, or the scaffold before the feature. Each checkpoint stands on its own and the sequence reads as an argument.
