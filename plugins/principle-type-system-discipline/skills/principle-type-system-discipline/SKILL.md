...
Applies to any typed language. If `$typescript-best-practices` is available for TypeScript work, apply it for concrete syntax; otherwise use the language's native sum types, narrowing, branding, parsing, and exhaustiveness checks.
...
- **External data is untyped until parsed.** RPC payloads, JSON, IPC messages, CLI args, config files, environment variables, database rows. Have a parse function at every boundary that turns unstructured input into the typed model. If `$principle-boundary-discipline` is available, apply it; otherwise validate once where data enters and trust the parsed type inside.
...
- **Derive types from authoritative schemas.** When a protocol buffer, OpenAPI spec, GraphQL schema, database migration, or design-system token file defines a shape, derive from it instead of hand-rolling a parallel type. Manual duplication drifts. If `$principle-encode-lessons-in-structure` is available, apply it; otherwise prefer generated types, lint rules, or canonical helpers over repeated textual instructions.
