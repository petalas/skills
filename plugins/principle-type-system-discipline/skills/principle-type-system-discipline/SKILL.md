...
- **Derive types from authoritative schemas.** When a protocol buffer, OpenAPI spec, GraphQL schema, database migration, or design-system token file defines a shape, derive from it instead of hand-rolling a parallel type. If `$principle-encode-lessons-in-structure` is available, apply it; otherwise prefer generated types, lint rules, or canonical helpers over repeated textual instructions.
