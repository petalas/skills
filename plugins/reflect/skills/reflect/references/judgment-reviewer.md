...
Look for mistakes and corrections, repeated user preferences, architectural facts, tool quirks, rationales, delegation friction, and manual steps that should become local automation.
...
- Principle: one durable rule.
- Evidence: an exact moment, short quote, command, or artifact pointer.
- Routing: the existing skill path and section, a description tune, or a new skill name.

Drop typos, retries, current revision identifiers, exact file counts, and anything already stated clearly in a skill the session followed.
