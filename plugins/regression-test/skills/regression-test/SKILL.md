...
This skill covers bug regressions only. A request for general TDD or test-first feature development belongs to a dedicated TDD workflow.
...
- If the bug exposes a broader class of failures, first add the focused regression path, then consider additional sibling coverage.
