...
If `$principle-type-system-discipline` is available, apply it first. Otherwise use this skill's rules directly: model valid states, parse external data at boundaries, avoid unsafe casts, and exhaust variants.
...
| Boundary validation   | Parse where data crosses in, into a named domain type. `Record<string, unknown>` (however spelled) stops at that parse. Trust types inside. If `$principle-boundary-discipline` is available, apply it; otherwise validate once at the boundary and trust the parsed type inside. |
