...
Code examples for each rule in `SKILL.md`. If `$principle-type-system-discipline` and `$principle-boundary-discipline` are available, use them for the language-agnostic reasoning. Otherwise follow the modeling and boundary rules summarized in `SKILL.md`.
...
Validate once where data crosses in; trust types inside. If `$principle-boundary-discipline` is available, apply its full boundary guidance. Otherwise keep parsing and error handling at the external edge and keep internal logic typed.
