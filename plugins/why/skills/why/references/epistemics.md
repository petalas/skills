...
Code doesn't carry its own motivation. You can read what code does; you can't read _why it exists_. That lives in commits, repository change records, local documents, tests, and runtime evidence, all incomplete, biased, and sometimes missing entirely. Pretending otherwise produces confident-sounding guesses that mislead the user.
...
- A local decision record that says "we chose this limit to keep peak memory below the worker budget"
...
- A commit message that says "switching to this approach since the old one was flaky in tests"
...
- The change record doesn't say why, but the error-tracking timeline starts just before the fix and stops after its release, so a hotfix origin is plausible.
- The function name suggests retry logic; the retry count is 3; this matches the repository's general convention of "3 retries" seen elsewhere in the codebase.
...
- "the project decided". Claims a durable decision was recorded
...
- The missing rationale should not be treated as fact
- They can either make a fresh decision from current constraints or decide the question is not worth pursuing further
