...
1. **Event usage trajectory.** Daily counts on the relevant `stg_*` model across a +/-30d window around the change. A step function from zero to steady volume within a day or two of the change is strong circumstantial evidence it launched the feature. A decay to zero suggests a deprecation or deletion.
...
- **Instrumented != caused.** An event's existence means it was worth logging, not that the target code exists _because_ of it. Pair it with a repository-history citation before claiming causation.
