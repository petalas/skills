...
- **Metrics.** Counters, gauges, and histograms instrumented in the system. A metric's _presence_ is itself evidence that the number was worth watching.
- **Monitors & alerts.** Conditions configured as operationally significant. A monitor firing on `rate_limit_hit > 10/min` is evidence that the threshold mattered.
- **Dashboards.** Curated views. The charts show which subsystem signals were considered important.
...
1. **Identify the owning service(s).**
   ...
   ```
   search_datadog_services (filter by service name)
   search_datadog_service_dependencies (see upstream/downstream)
   ```

2. **Dashboards and monitors first. They show what the system is designed to watch.**
...
6. **Incidents.**
   ...
   ```
   search_datadog_incidents (by title and date range)
...
- **Overfitting to the chart you found.** A chart reflects its creator's framing. A chart named "retry success rate" is evidence that retry success was monitored, not that it motivated a specific line of code.
